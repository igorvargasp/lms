...
export const registrationUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isEmailExist)
        throw new ErrorHandler("This email already exists", 409);
...
export const activateUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // set user status to active and save the user in database
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (email && user) {
        ...
        if (isEmailExist)
          return next(
            new ErrorHandler("This Email has already been used", 400)
          );
        user.email = email;
      }
      if (name && user) {
